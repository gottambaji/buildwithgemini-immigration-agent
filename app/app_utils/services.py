...
def get_memory_service():
    """Process-wide memory service: VertexAiMemoryBankService when memory_bank_id or GOOGLE_CLOUD_AGENT_ENGINE_ID is set."""
    if memory_bank_id := (os.environ.get("MEMORY_BANK_ID") or os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID")):
        from google.adk.memory import VertexAiMemoryBankService

        # Runtime-injected agent-engine region, not GOOGLE_CLOUD_LOCATION (which is pinned to "global")
        location = os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION") or "us-east1"
        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT") or "qwiklabs-gcp-03-76f4257706a9",
            location=location,
